...
class Game {
	constructor(p1, p2, height = 6, width = 7) {
		...
		this.height = height;
		this.width = width;
		//because player 1 starts the game
		this.currPlayer = p1;
		//these can moved out of constructor
		this.makeBoard();
		this.makeHtmlBoard();
	...
	makeHtmlBoard() {
		...
		//use bind to remove the event listener later
	...
	handleClick(evt) {
		...
		// check for win
		if (this.checkForWin()) {
			//property 'game over' now becomes true
			this.gameOver = true;
			//add new color to curr player
	...
	checkForWin() {
		const _win = (cells) =>
			...
			cells.every(
				([ y, x ]) => y >= 0 && y < this.height && x >= 0 && x < this.width && this.board[y][x] === currPlayer
			);
...
document.getElementById('start').addEventListener('click', () => {
	let p1 = new Player(document.getElementsById('player_1').value);
	let p2 = new Player(document.getElementById('player_2').value);
	new Game(p1, p2);
});
